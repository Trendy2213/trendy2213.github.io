...
document.querySelector('.login form').onsubmit=async event=>{event.preventDefault();const f=event.currentTarget;const feedback=f.querySelector('.feedback');try{const result=await signInWithEmailAndPassword(auth,f.email.value.trim(),f.password.value);if(result.user.email.toLowerCase()!==ADMIN){await signOut(auth);throw new Error('Cuenta no autorizada.')}feedback.textContent='';}catch(e){feedback.textContent=e.message==='Cuenta no autorizada.'?e.message:'Correo o contraseña incorrectos.'}};
